...
4. **Delivery**: in S3 mode the WebP is uploaded to the target bucket; in local mode it stays in the run's output directory (`<work_dir>/<uuid>/output`), which `run.sh` collects into `./test/output`
